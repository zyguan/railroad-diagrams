(function (global, factory) {
  ...
  */
  ...
  _exports.defaultCSS = _exports.default = _exports.ZeroOrMore = _exports.Terminal = _exports.Start = _exports.Stack = _exports.Skip = _exports.Sequence = _exports.Path = _exports.Options = _exports.OptionalSequence = _exports.Optional = _exports.OneOrMore = _exports.NonTerminal = _exports.MultipleChoice = _exports.HorizontalChoice = _exports.Group = _exports.FakeSVG = _exports.End = _exports.DiagramMultiContainer = _exports.Diagram = _exports.ComplexDiagram = _exports.Comment = _exports.Choice = _exports.Block = _exports.AlternatingSequence = void 0;
  const funcs = {};
  var _default = funcs;
  ...
  _exports.defaultCSS = defaultCSS;

  class FakeSVG {
    constructor(tagName, attrs, text) {
      if (text) this.children = text;else this.children = [];
  ...
  _exports.DiagramMultiContainer = DiagramMultiContainer;

  class Diagram extends DiagramMultiContainer {
    constructor(...items) {
      super('svg', items, {
        class: Options.DIAGRAM_CLASS
      });
  ...
  class ComplexDiagram extends FakeSVG {
    constructor(...items) {
      ...
      diagram.items[0] = new Start({
        type: "complex"
      });
      diagram.items[diagram.items.length - 1] = new End({
        type: "complex"
      });
      return diagram;
    }
  ...
  _exports.ComplexDiagram = ComplexDiagram;
    ...
    format(x, y, width) {
      ...
      for (var i = 0; i < this.items.length; i++) {
        ...
        if (i !== this.items.length - 1) {
          new Path(x, y).arc('ne').down(Math.max(0, item.down + Options.VS - Options.AR * 2)).arc('es').left(innerWidth).arc('nw').down(Math.max(0, this.items[i + 1].up + Options.VS - Options.AR * 2)).arc('ws').addTo(this);
          y += Math.max(item.down + Options.VS, Options.AR * 2) + Math.max(this.items[i + 1].up + Options.VS, Options.AR * 2); //y += Math.max(Options.AR*4, item.down + Options.VS*2 + this.items[i+1].up)
  ...
  class OptionalSequence extends DiagramMultiContainer {
    ...
    format(x, y, width) {
      ...
      for (var i = 0; i < this.items.length; i++) {
        ...
        if (i === 0) {
          // Upper skip
          new Path(x, y).arc('se').up(y - upperLineY - arc * 2).arc('wn').right(itemWidth - arc).arc('ne').down(y + item.height - upperLineY - arc * 2).arc('ws').addTo(this); // Straight line

          new Path(x, y).right(itemSpace + arc).addTo(this);
          item.format(x + itemSpace + arc, y, item.width).addTo(this);
          x += itemWidth + arc;
          y += item.height; // x ends on the far side of the first element,
          // where the next element's skip needs to begin
        } else if (i < last) {
          // Upper skip
          new Path(x, upperLineY).right(arc * 2 + Math.max(itemWidth, arc) + arc).arc('ne').down(y - upperLineY + item.height - arc * 2).arc('ws').addTo(this); // Straight line

          new Path(x, y).right(arc * 2).addTo(this);
          item.format(x + arc * 2, y, item.width).addTo(this);
          new Path(x + item.width + arc * 2, y + item.height).right(itemSpace + arc).addTo(this); // Lower skip

          new Path(x, y).arc('ne').down(item.height + Math.max(item.down + Options.VS, arc * 2) - arc * 2).arc('ws').right(itemWidth - arc).arc('se').up(item.down + Options.VS - arc * 2).arc('wn').addTo(this);
          ...
          new Path(x, y).right(arc * 2).addTo(this);
          item.format(x + arc * 2, y, item.width).addTo(this);
          new Path(x + arc * 2 + item.width, y + item.height).right(itemSpace + arc).addTo(this); // Lower skip
  ...
  _exports.OptionalSequence = OptionalSequence;
  ...
  class AlternatingSequence extends DiagramMultiContainer {
    ...
    format(x, y, width) {
      ...
      new Path(x + this.width - 2 * arc, y - firstOut).arc('ne').down(firstOut - 2 * arc).arc('ws').addTo(this); // bottom
      ...
      new Path(x + this.width - 2 * arc, y + secondOut).arc('se').up(secondOut - 2 * arc).arc('wn').addTo(this); // crossover
      ...
      new Path(x + arc, y - crossY / 2 - arc).arc('ws').right(crossBar).arc_8('n', 'cw').l(crossX - arcX, crossY - arcY).arc_8('sw', 'ccw').right(crossBar).arc('ne').addTo(this);
      new Path(x + arc, y + crossY / 2 + arc).arc('wn').right(crossBar).arc_8('s', 'ccw').l(crossX - arcX, -(crossY - arcY)).arc_8('nw', 'cw').right(crossBar).arc('se').addTo(this);
      return this;
    }

  }

  _exports.AlternatingSequence = AlternatingSequence;
  ...
  class Choice extends DiagramMultiContainer {
    constructor(normal, ...items) {
      ...
      for (var i = first; i < normal; i++) {
        if (i == normal - 1) arcs = Options.AR * 2;else arcs = Options.AR;
    ...
    format(x, y, width) {
      ...
      x += gaps[0];
      var last = this.items.length - 1;
      var innerWidth = this.width - Options.AR * 4; // Do the elements that curve above
      ...
      new Path(x + Options.AR * 2 + innerWidth, y + this.height).right(Options.AR * 2).addTo(this); // Do the elements that curve below
  ...
  class HorizontalChoice extends DiagramMultiContainer {
    constructor(...items) {
      ...
      this.height = 0; // All but the last have a track running above them

      this._upperTrack = Math.max(Options.AR * 2, Options.VS, max(allButLast, x => x.up) + Options.VS);
      this.up = Math.max(this._upperTrack, last.up); // All but the first have a track running below them
    ...
    format(x, y, width) {
      ...
      const allButLast = this.items.slice(0, -1); // upper track

      var upperSpan = sum(allButLast, x => x.width + (x.needsSpace ? 20 : 0)) + (this.items.length - 2) * Options.AR * 2 - Options.AR;
      new Path(x, y).arc('se').v(-(this._upperTrack - Options.AR * 2)).arc('wn').h(upperSpan).addTo(this); // lower track

      var lowerSpan = sum(allButFirst, x => x.width + (x.needsSpace ? 20 : 0)) + (this.items.length - 2) * Options.AR * 2 + (last.height > 0 ? Options.AR : 0) - Options.AR;
      var lowerStart = x + Options.AR + first.width + (first.needsSpace ? 20 : 0) + Options.AR * 2;
      new Path(lowerStart, y + this._lowerTrack).h(lowerSpan).arc('se').v(-(this._lowerTrack - Options.AR * 2)).arc('wn').addTo(this); // Items
      ...
      for (const [i, item] of enumerate(this.items)) {
        ...
        if (i === 0) {
          ...
          new Path(x, y - this._upperTrack).arc('ne').v(this._upperTrack - Options.AR * 2).arc('ws').addTo(this);
          x += Options.AR * 2;
        } // item
        ...
        if (i === this.items.length - 1) {
          ...
          if (item.height - this._lowerTrack >= Options.AR * 2) {
            new Path(x, y + item.height).arc('se').v(this._lowerTrack - item.height + Options.AR * 2).arc('wn').addTo(this);
          } else {
          ...
          new Path(x, y + item.height).arc('ne').v(this._lowerTrack - item.height - Options.AR * 2).arc('ws').addTo(this);
        }
      }
  ...
  _exports.HorizontalChoice = HorizontalChoice;
  ...
  class MultipleChoice extends DiagramMultiContainer {
    constructor(normal, type, ...items) {
      ...
      for (var i = 0; i < this.items.length; i++) {
        let item = this.items[i];
        let minimum;
        if (i == normal - 1 || i == normal + 1) minimum = 10 + Options.AR;else minimum = Options.AR;
    ...
    format(x, y, width) {
      ...
      new Path(x + gaps[0] + this.width, y + this.height).right(gaps[1]).addTo(this);
      x += gaps[0];
      var normal = this.items[this.normal]; // Do the elements that curve above
      ...
      for (var i = this.normal - 1; i >= 0; i--) {
        ...
        new Path(x + 30, y).up(distanceFromY - Options.AR).arc('wn').addTo(this);
        item.format(x + 30 + Options.AR, y - distanceFromY, this.innerWidth).addTo(this);
        new Path(x + 30 + Options.AR + this.innerWidth, y - distanceFromY + item.height).arc('ne').down(distanceFromY - item.height + this.height - Options.AR - 10).addTo(this);
      ...
      for (i = this.normal + 1; i < this.items.length; i++) {
        ...
        new Path(x + 30, y).down(distanceFromY - Options.AR).arc('ws').addTo(this);
        item.format(x + 30 + Options.AR, y + distanceFromY, this.innerWidth).addTo(this);
        new Path(x + 30 + Options.AR + this.innerWidth, y + distanceFromY + item.height).arc('se').up(distanceFromY - Options.AR + item.height - normal.height).addTo(this);
  ...
  class Optional extends FakeSVG {
    constructor(item, skip) {
      if (skip === undefined) return new Choice(1, new Skip(), item);else if (skip === "skip") return new Choice(0, new Skip(), item);else throw "Unknown value for Optional()'s 'skip' argument.";
    }
  ...
  class OneOrMore extends FakeSVG {
    ...
    format(x, y, width) {
      ...
      new Path(x + this.width - Options.AR, y + this.height).right(Options.AR).addTo(this); // Draw repeat arc
  ...
  _exports.ZeroOrMore = ZeroOrMore;
  ...
  class Group extends FakeSVG {
    constructor(item, label) {
      super('g');
      this.item = wrapString(item);
      this.label = label instanceof FakeSVG ? label : label ? new Comment(label) : undefined;
  ...
  class Terminal extends FakeSVG {
    ...
    format(x, y, width) {
      ...
      var text = new FakeSVG('text', {
        x: x + this.width / 2,
        y: y + 4
      }, this.text);
      if (this.href) new FakeSVG('a', {
  ...
  function SVG(name, attrs, text) {
    ...
    for (var attr in attrs) {
      if (attr === 'xlink:href') el.setAttributeNS("http://www.w3.org/1999/xlink", 'href', attrs[attr]);else el.setAttribute(attr, attrs[attr]);
    }
